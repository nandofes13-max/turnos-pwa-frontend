...
export default function Inicio() {
  ...
  // AHORA: No depende de isInstallable, solo de que no esté instalada
  useEffect(() => {
  ...
  const handleInstall = async (): Promise<boolean> => {
    ...
    // Si no hay prompt disponible, mostrar instrucciones según el navegador
    ...
    if (isIOS) {
      ...
      alert('🦊 Firefox no soporta instalación automática.\n\nUsa Chrome o Edge para instalar la aplicación, o accede desde tu teléfono.');
    } else if (isBrave) {
      alert('🛡️ Brave bloquea la instalación automática por privacidad.\n\nUsa el botón de instalación en la barra de direcciones o prueba con Chrome.');
    } else {
      alert('💡 Para instalar la aplicación:\n\nBusca el icono de instalación (⬇️ o +) en la barra de direcciones de tu navegador.');
    }
      ...
      <div className={styles['inicio-left']}>
        <div className={styles['inicio-left-content']}>
          ...
          <div className={styles['inicio-card']}>
            ...
            {/* AHORA: Visible siempre que no esté instalada y el modal no esté abierto */}
            <BannerInstall
              isVisible={!isInstalled && !isModalOpen}
              onInstall={handleInstall}
              isInstalled={isInstalled}
            />
