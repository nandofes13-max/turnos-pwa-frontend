...
const RELACIONES_URL = `${API_BASE_URL}/negocio-actividades`;
const NEGOCIOS_URL = `${API_BASE_URL}/negocios`;
const ACTIVIDADES_URL = `${API_BASE_URL}/actividades`;

export default function NegocioActividades() {
  ...
  const confirmarReactivar = async () => {
    ...
    try {
      const res = await fetch(`${RELACIONES_URL}/reactivar/${confirmReactivar.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' }
      });
  ...
  return (
    <div className="tm-page">
      <h1 className="tm-titulo">Gestión de Actividades por Negocio</h1>
